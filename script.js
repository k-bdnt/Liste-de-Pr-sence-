...
const plannedAbsenceKey = 'plannedAbsence';
...
function calculateBreakMinutes(breakStart, breakEnd) {
    return timeDiffInMinutes(breakStart, breakEnd);
}
...
function getWorkersList() {
    return JSON.parse(localStorage.getItem(workersKey) || '[]');
}
...
function openTab(evt, tabName) {
    ...
    if (tabName === 'employees') {
        ...
        loadPendingActions();
    } // NOUVEAU: Appel des fonctions du Dashboard
    if (tabName === 'register') {
        loadWorkersIntoSelects();
        loadQuickSummary();
        loadPlannedAbsence();
...
function loadWorkers() {
    ...
    const workers = getWorkersList();
    if (workers.length === 0) {
        ul.innerHTML = '<li style="color: #777; border-left: none; background-color: transparent;">Aucun employé enregistré.</li>';
        return;
    }
    workers.forEach((w, i) => {
        const name = w.name;
...
function loadHoliday() {
    ...
    if (holidays.length === 0) {
        ul.innerHTML = '<li style="color: #777; border-left: none; background-color: transparent;">Aucun jour férié enregistré.</li>';
        return;
    }
...
function loadQuickSummary() {
    const ul = document.getElementById('quickSummaryList');
    ul.innerHTML = '';
    const presence = JSON.parse(localStorage.getItem(presenceKey) || '[]');
...
function loadPlannedAbsence() {
    const ul = document.getElementById('plannedAbsenceList');
    ul.innerHTML = '';
